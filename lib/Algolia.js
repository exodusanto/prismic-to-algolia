...
class Algolia {
  ...
  constructor (config, index) {
    ...
    this.index.getSettings((err, content) => {
      ...
      let searchableAttributes = content.searchableAttributes 
        && content.searchableAttributes.slice(0) || [];
      ...
      if(searchableAttributes.indexOf("lang") === -1) 
        searchableAttributes.push("lang");

      if(searchableAttributes.length !== content.searchableAttributes.length)
   ...
   */
  _getObjectById (id, lang) {
    return new Promise((resolve) => {
      this.index.search({
        query: `${id} ${lang}`,
        restrictSearchableAttributes: ["id", "uid", "lang"]
      }, (error, results) => {
        if (error) {
          return console.error(error);
        }
